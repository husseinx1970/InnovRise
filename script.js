...
// ---- Neural Particle Playground (scoped to the panel size) ----
...
if (C){
  ...
  // Resize observer to handle panel changes
  new ResizeObserver(resize).observe(C);
  window.addEventListener('resize', resize, {passive:true});
  // initial size after DOM paints
  setTimeout(resize, 0);
  ...
  function field(x,y,t){
    ...
    let fy = m*0.6 - n*0.4;
    if(pointer.active){
      const dx = x - pointer.x, dy = y - pointer.y, d2 = dx*dx + dy*dy + 40;
      const k = 1200/d2;
      fx -= dx * k; fy -= dy * k;
    }
  ...
  function draw(tms){
    const t = tms/1000;
    ctx.fillStyle = 'rgba(11,15,22,0.22)';
    ...
    for(const p of parts){
      ...
      if(idle>0){
        ...
        const targetR = ((p.x*0.37 + p.y*0.23) % 2) > 1 ? Math.min(W,H)*0.22 : Math.min(W,H)*0.42;
        const pull = (targetR - r)*0.0009*idle;
      ...
      if(pointer.burst>0){
        ...
        const imp = 1400/d2 * pointer.burst;
