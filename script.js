...
// ====== Interactive "InnovRise" particles ======
const canvas = document.getElementById('playground');
if (canvas){
  const ctx = canvas.getContext('2d');
  let W, H, DPR;
  const particles = [];
  const N = 1100;                // number of particles
  let targets = [];              // sampled points of the word
  let jitter = 0;                // soft motion around targets
  let mode = 'text';             // 'text' | 'free' (after explode)
  let explodeUntil = 0;          // timestamp until which particles remain more "free"
  ...
  // Build word targets with fit-to-box
  function buildTextTargets(){
    ...
    for (let j=0;j<off.height;j+=step){
      for (let i=0;i<off.width;i+=step){
        ...
        if (img[idx] > 0){
          // small random offset for a living feel
  ...
  // Event handlers
  canvas.addEventListener('mouseenter', (e)=>{ pointer.inside = true; });
  canvas.addEventListener('mouseleave', (e)=>{ pointer.inside = false; });
  ...
  function draw(){
    ...
    const now = performance.now();
    if (mode === 'free' && now > explodeUntil){
      mode = 'text'; // return to text after explosion
    }
    ...
    for (let i=0;i<particles.length;i++){
      ...
      // Target = either the text point or the pointer
      if (pointer.inside && mode !== 'free'){
        // attract to pointer
        const dx = (pointer.x + Math.sin(jitter + i*0.03)*0.5) - p.x;
        const dy = (pointer.y + Math.cos(jitter*1.2 + i*0.03)*0.5) - p.y;
        p.vx += dx * 0.05; p.vy += dy * 0.05;
      } else {
        // attract to word target
        ...
        if (T){
          ...
          const k = (mode === 'free') ? 0.02 : 0.06; // slower pull during explode decay
          p.vx += dx * k; p.vy += dy * k;
        }
      }
